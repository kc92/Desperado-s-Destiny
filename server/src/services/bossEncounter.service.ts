  ...
  private static async awardVictoryRewards(
    ...
    for (const [charId, state] of encounter.playerStates.entries()) {
      ...
      const goldReward = Math.floor(
        (boss.goldReward.min + Math.random() * (boss.goldReward.max - boss.goldReward.min)) /
      ...
      if (isFirstKill && boss.firstKillBonus) {
        await GoldService.addGold(
          ...
          TransactionSource.BOSS_VICTORY,
          { bossId: boss.id, bossName: boss.name, firstKill: true },
          session
        );
      ...
      characterRewards.set(charId, {
        gold: goldReward + (isFirstKill && boss.firstKillBonus ? boss.firstKillBonus.gold || 0 : 0),
        experience: xpReward,
        items: [], // TODO: Roll loot table
    ...
    const result: BossEncounterResult = {
      ...
      highestSingleHit: 0, // TODO: Track from rounds
      participants,
      rewards: {
